...
const React = require('react'),
  { Component, useEffect } = React

const { Animated, Image, Dimensions, PanResponder, StyleSheet, TouchableWithoutFeedback, View, Text } = require('react-native')
...
const expandColor = (color) =>
  typeof color == 'string' && color.length === 4 ? `#${color[1]}${color[1]}${color[2]}${color[2]}${color[3]}${color[3]}` : color
...
module.exports = class ColorPicker extends Component {
  ...
  wheelPanResponder = PanResponder.create({
    ...
    onPanResponderRelease: (event, gestureState) => {
      ...
      const { h, s, v } = hsv
      if (!this.props.noSnap && radius <= 0.1 && radius >= 0) this.animate('#ffffff', 'hs', false, true)
          ...
          <Animated.View
            style={[
              ss.swatchTouch,
              {
                backgroundColor: c,
                transform: [
                  { scale: this.swatchAnim[i].interpolate({ inputRange: [0, 0.5, 1], outputRange: [0.666, 1, 0.666] }) }
                ]
              }
            ]}
          />
            ...
            <Animated.View
              style={[
                ss.swatchTouch,
                {
                  ...
                  transform: [
                    { scale: this.discAnim[i].interpolate({ inputRange: [0, 0.5, 1], outputRange: [0.666, 1, 0.666] }) }
                  ]
                }
              ]}
            >
  ...
  render() {
    const { style, thumbSize, sliderSize, gapSize, swatchesLast, swatchesOnly, sliderHidden, discrete, row } = this.props
      ...
      <View style={[ss.root, row ? { flexDirection: 'row' } : {}, style]}>
        ...
        {!swatchesOnly && (
          <View style={[ss.wheel]} key={'$1'} onLayout={this.onSquareLayout}>
            {this.wheelWidth > 0 && (
              <View style={[{ padding: thumbSize / 2, width: this.wheelWidth, height: this.wheelWidth }]}>
